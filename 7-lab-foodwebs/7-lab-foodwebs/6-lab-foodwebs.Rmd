...
```{r create-adj-mat-2}
car_mat <- matrix(carnivores, nrow = length(carnivores), ncol = length(carnivores), byrow = TRUE)

filtered_mat <- adj_matrix * car_mat
filtered_mat
col_sums1 <- colSums(filtered_mat)
col_sums1

# same thing? 

adj_matrix2 <- sweep(adj_matrix, 2, carnivores, `*`)
print(adj_matrix2)
row_sums <- rowSums(adj_matrix2)
col_sums <- colSums(adj_matrix2)
row_sums
col_sums
...
adj_matrix_time2 <- adj_matrix

# Zero out rows and columns of extinct species
adj_matrix_time2[!present_species, ] <- 0
adj_matrix_time2[, !present_species] <- 0

# Check the number of rows with probabilities > 0
nonzero_rows <- rowSums(adj_matrix_time2) > 0
nonzero_row_count <- sum(nonzero_rows)


cat("the number of rows with probabilities > 0 is", nonzero_row_count, "\n")
cat("the number of species present is", sum(present_species), "\n")
