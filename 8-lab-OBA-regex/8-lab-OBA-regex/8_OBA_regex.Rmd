...
oba$plantResolution <- sapply(oba$Associated.plant, function(x){
...
oba <- oba %>%
  mutate(plantTaxaWordCount = str_count(Associated.plant, "\\S+"))
...
for (i in 1:nrow(oba)) {
  if (grepl("aceae$", oba$Associated.plant[i], ignore.case = TRUE)) {
    # If the name ends with "aceae", it's a family
    oba$plantResolution[i] <- "Family"
  } else if(grepl("\\S+\\s+\\S+", oba$Associated.plant[i]) && !grepl("sp\\.$", oba$Associated.plant[i])){
    oba$plantResolution[i] <- "Species"
  } else if(grepl("^\\S+$", oba$Associated.plant[i])){
    oba$plantResolution[i] <- "Genus"
  }
  # } else if grepl(oba$plantTaxaWordCount[i] == 1) {
  #   # If there is only one word, it's a genus
  #   oba$plantResolution[i] <- "Genus"
  # } else if (oba$plantTaxaWordCount[i] >= 2 && !grepl("sp\\.$", oba$Associated.plant[i], ignore.case = TRUE)) {
  #   # If there are two or more words and does not end with "sp.", it's a species
  #   oba$plantResolution[i] <- "Species"
  # }
}
...
oba$Associated.plant <- ifelse(
  is.na(oba$Associated.plant), 
  NA, 
  gsub("^([A-Za-z]+)$", "\\1 sp.", oba$Associated.plant)
)
...
# Create plantGenus column
oba$plantGenus <- ifelse(
  grepl("^[A-Za-z]+(\\s[A-Za-z]+)?$", oba$Associated.plant), # Check for genus or species (1-2 words)
  sub(" .*", "", oba$Associated.plant),                      # Extract the first word (genus)
  NA                                                          # Assign NA if it's family level
)


## To check finish with
print(oba$plantGenus[1:100])
```
