...
for (i in 1:nrow(oba)) {
  if (grepl("aceae$", oba$Associated.plant[i], ignore.case = TRUE)) {
    ...
    # If it doesn't meet any conditions, leave as NA
    oba$plantResolution[i] <- NA
  }
}
...
oba$Associated.plant <- gsub("^([A-Za-z]+)$", "\\1 sp.", oba$Associated.plant)

data$Associated.plant <- str_replace_all(data$Associated.plant, "^([A-Za-z]+)$", "\\1 sp.")
